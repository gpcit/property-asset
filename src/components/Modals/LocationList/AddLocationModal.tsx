...
const AddLocationModal = ({ modalOpen, setModalOpen }: ChildrenModalProps) => {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        try {
            const respose = await createLocation(formData);
            fetchLocations();
            setModalOpen(false);
        } catch (error) {
  ...
  return (
    <MainModal modalOpen={modalOpen} setModalOpen={setModalOpen}>
        <div className='inline-block transitions inset-0 sm:w-4/5 border shadow-xl rounded-2xl md:w-3/5 lg:w-2/5 w-full align-middle p-10 transform  h-full bg-darkColor'>
            <h2 className='text-3xl font-bold text-white'> Add Location </h2>
            <form onSubmit={handleSubmit} className='grid grid-cols-6 gap-6 text-left mt-6'>
                <div className='col-span-6 text-black'>
                    <Input fontColor='text-white' name='name' label={'Location Name'} placeholder="Location Name" type="text"  onChange={handleChange} />
                </div>
