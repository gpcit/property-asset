...
import { createCompanyPlant } from '@/services/companyPlantServices'
import { useCompanyPlantStore } from '@/stores/companyPlantStore'

const AddCompanyPlantModal = ({ modalOpen, setModalOpen }: ChildrenModalProps) => {
    const { fetchAllCompanyPlant} = useCompanyPlantStore()
    const [error, setError] = useState<string>('');
    const [formData, setFormData] = useState<CompanyPlantProps>({});
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        try {
            const respose = await createCompanyPlant(formData);
  ...
  return (
    <MainModal modalOpen={modalOpen} setModalOpen={setModalOpen}>
        <div className='inline-block transitions inset-0 sm:w-4/5 border shadow-xl rounded-sm md:w-3/5 lg:w-2/5 w-full align-middle p-10 transform  h-full bg-darkColor'>
            <h2 className='text-3xl font-bold text-white'> Add Company Plant </h2>
            <form onSubmit={handleSubmit} className='grid grid-cols-6 gap-6 text-left mt-6'>
                <div className='col-span-6 text-black'>
                    <Input fontColor='text-white' name='name' label={'Company Plant'} placeholder="Company Plant" type="text"  onChange={handleChange} />
                </div>
...
export default AddCompanyPlantModal